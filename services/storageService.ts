...
export const storageService = {
  ...
  getAllTransferChecks: async (): Promise<DataTransferCheck[]> => {
    try {
      // Get all projects first, then get all transfer checks
      const projects = await api.projects.getAll();
      const allChecks: DataTransferCheck[] = [];

      for (const project of projects) {
        const checks = await api.dataTransfer.getByProject(project.projectId);
  ...
  getAllIssues: async (): Promise<MigrationIssue[]> => {
    try {
      ...
      for (const project of projects) {
        const issues = await api.issues.getByProject(project.projectId);
